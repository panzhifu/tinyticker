...
use crate::widget::{DRAW_INTERVAL, LOGICAL_SIZE, Frame, Widget};
...
/// libwayland 的 `f`（`wl_fixed_t`）是 16.16 定点数，1.0 = 65536。
fn fixed(v: i32) -> f64 {
    v as f64 / 65536.0
}
...
client.commit();
    ...
    /// 事件循环：每 DRAW_INTERVAL 至少推进一次，其余时间阻塞在 Wayland socket 上。
    fn event_loop(
        &mut self,
        cmd_rx: &Receiver<Command>,
        handle_rx: &Receiver<TrayHandle>,
