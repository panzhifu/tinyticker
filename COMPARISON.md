...
| **交互** | | |
| 托盘图标 | ✅（0.2.0 起程序化绘制 32x32；工作区版本改为每秒重绘，指针按本地时间摆） | ✅（可换 GIF/WebP/图片动画） |
...
| 优先级 | 功能 | 说明 | 预估成本 |
|---|---|---|---|
| ★☆☆ | 托盘动画（GIF / WebP） | 系统状态那半边已做（见上表）；剩下的是让用户自带动图。Catime 自己不解码，全交给 Windows 的 WIC，Linux 无等价物，只能自写 GIF 的 LZW（~200 行）；WebP 要整个 VP8 codec，不划算 | 中高 |
