...
| 番茄钟 | ✅（0.4.0 专注/休息自动轮转 + 轮数显示；工作区版本补上**长休息与组数上限**：`pomo_work` / `pomo_break` / `pomo_long_break` / `pomo_rounds` / `pomo_cycles`；2026-09-25 再补 **`pomo_seq`**——`25m,5m,15m` 这样自己排一串任意段（≤16 段），跑完整条算一遍、`pomo_cycles` 限定跑几遍，状态行换成 `POMO n`；同日补上**托盘半边**：「🍅 番茄分段」每段一项、当前段打勾、点了跳去那段；2026-09-26 子菜单恒建并加「✏ 编辑分段」——输入行键入 `25m,5m,15m` 整条替换，就地生效） | ✅（`POMODORO_TIME_OPTIONS` 序列 ≤10 段 + `POMODORO_LOOP_COUNT`，默认 1 组；每段还能在对话框里单独重设时长——单段对话框那半边我们仍无，序列整串编辑已补） |
