...
| 托盘实时显示 CPU/内存/电池 | ✅（工作区版本：`tray_icon = clock\|cpu\|memory\|battery`，内盘自底向上涨水位，每秒重绘并发 `NewIcon`；数据来自 `/proc/stat` 差分、`MemAvailable`、`/sys/class/power_supply`，未引入任何 crate） | ✅ |
| 全局快捷键 | ❌ | ✅ |
| 右键关闭窗口 | ✅ | 编辑模式下右键退出编辑 |
| **计时输入** | | |
...
| 优先级 | 功能 | 说明 | 预估成本 |
|---|---|---|---|
| ★☆☆ | 托盘动画（GIF / WebP） | 系统状态那半边已做（见上表）；剩下的是让用户自带动图，GIF 的 LZW 解码需自写（~200 行），WebP 更贵 | 中高 |
