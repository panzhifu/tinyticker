...
| **视觉自定义** | | |
| 颜色自定义 | ✅（0.2.0 配置文件设四色；工作区版本加托盘「外观 → 配色预设」6 套，点击即时重绘并写回配置。仍无调色板） | ✅ 15 种预设 + 调色板 + 实时预览 |
| 字体 | 内置 8x8 位图字体（仅 ASCII，不支持中文显示） | 多款内置等宽字体 + 自定义字体 + 字体精简工具 |
| 字号缩放 | ✅ 跟随 DPI + 滚轮自由调节（0.4.0）；工作区版本起在**托盘图标上滚动**同样缩放（SNI `Scroll`） | 滚轮自由调节 |
| **交互** | | |
| 托盘图标 | ✅（0.2.0 起程序化绘制 32x32；工作区版本按派发节拍重绘、指针按本地时间摆，并支持用户自带 **GIF** 动图） | ✅（可换 GIF/WebP/图片动画） |
| 托盘控制开始/暂停/重置 | ✅（工作区版本：左键 = 开始/暂停，中键 = 重置，右键 = 菜单；`ItemIsMenu` 因此报 `false`） | ✅ |
| 左键托盘直接输入时长 | ❌（需要文本输入框，本项目只有 8x8 位图渲染器；改走托盘预设 + 命令行参数） | ✅（左键弹出时间设置框） |
...
| 快速预设时长 | ✅（0.2.0 托盘二级菜单 1/5/15/25/45/60 分钟；工作区版本档位可配 `presets`，最多 24 段、标签自动生成） | ✅（最多 50 段、单段上限 86400 秒） |
| **计时结束动作** | | |
| 颜色/状态提示 | ✅（变绿 + DONE） | ✅ |
...
| 插件系统 | ⚠️ 只读半边（工作区版本：`text_source` 指向的文件第一行顶替状态行，带 64 KB 上限与 (大小,mtime) 变化检测；**不替你启动插件脚本**，生产者自己跑） | ✅（output.txt 文件通信 + 自动拉起脚本） |
| 国际化 | 无 UI 文本 | ✅ 多语言 |
