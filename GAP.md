...
>
> **取证基准**
> - tinyticker：`main` 分支工作区（v0.6.0：第二梯队、零散 XS 整批、A 类声音/PNG 批三批已并入发布；其后清了 **R1 键盘通路整批**（§零/§十二）与 **输入行三模式批**（§四/§八/§十三）），**247** 项内联测试全通过、`cargo clippy --all-targets -- -D warnings` 零告警；`cargo build --release` 实测 **696 552 B**（0.664 MiB），`--no-default-features` 687 584 B（0.656 MiB）——A 类那批 +35 KB、R1 键盘批 +8.5 KB（dlopen libxkbcommon + 两后端键盘事件 + 输入行状态机 + CLI `--input`）、三模式批 **+3.2 KB**（颜色/分段/预设三个模式 + 三个子菜单入口 + SyncConfig 补预设热重建）。字形层、套接字与各批格式/图标/语言/分页项见 §5.1、§九 与各表的"已做"标注。
...
**托盘那半边已做（2026-09-25）**：配了 `pomo_seq` 时根菜单多一个「🍅 番茄分段」——每段一项（"第 n 段 · 25 分"），**当前段打勾**，点了跳去那段（`Timer::goto_step`：换读数不动运行态）。段号的真值从主循环经 `TrayMsg::SyncPomo` 回填（只在变化时推一次），不跟点击走——跳段之后当前段才跟着走，那才是勾该待的地方。~~仍然只能整串从配置文件改序列~~ **就地编辑已做（2026-09-26）**：「🍅 番茄分段」子菜单恒建（空序列也在——编辑项正是从经典配方到任意序列的门），头一项「✏ 编辑分段…」打开输入行的分段模式，`25m,5m,15m` 回车整条替换、SyncConfig 当场重建段列表；非法整条不采信（与配置文件同一把尺子 `parse_span_list`）。
...
| 配置热加载 | 独立线程 `FindFirstChangeNotificationW` 监听**目录**，200 ms 去抖，再按 `(存在,mtime,大小)` 三元组复核，确认真改了才 `WM_APP_CONFIG_CHANGED` 全量重应用（`config_watcher_thread.c:104-183`）；另有 INI 层 100 ms 节流的 stat 兜底（`config_ini_read.c:8-12`）——手改文件即时生效，含颜色/特效/字体/时补/预设/番茄序列/通知/托盘动画 | **已做（2026-09-24）**：`config::Watch` 每拍一次 `stat`，靠 `(大小, mtime)` 认变更，真变了才 `Config::load()` 并当场应用（`widget.rs:reload_config`）。**只做兜底那一套**：Catime 自己也是"目录 watcher + stat 兜底"并行，我们只抄后者——延迟已经压在一个心跳以内，而代价是一个 syscall，不必为 inotify 新开一组 libc FFI、一条阻塞线程和它的 fd 生命周期 | 剩下两条真实差距：① **跑动中的计时器不跟文件走**（模式与总时长只在它空着时应用——改配置文件不该掐掉用户正在用的那个倒计时）；② ~~预设档位 / 托盘图标档 / 窗口位置仍要重启~~ **预设档位已热（2026-09-26）**：`SyncConfig` 处理器补上 `s.presets = cfg.presets.clone()`（此前只回填 `pomo_seq`，预设列表是启动快照），档位列表与番茄分段同为菜单结构、跟着重建；托盘图标档与窗口位置仍要重启（图标每拍从 state 取所以其实也热了，剩下的是 GIF 解码器与 surface 归后端那两样）。改到时打一行提示，不是静默失效 |
...
>
> **同日（三模式批）**：R1 的红利立刻兑了三笔——输入行从单一时长模式扩成四模式（[`InputMode`]，`widget.rs`）：`# ` 颜色（Gradient 全语法落到运行色，入口在「外观 ▸ 文字颜色」头一项）、`% ` 番茄分段（`25m,5m,15m` 整条替换 `pomo_seq`，入口在「🍅 番茄分段」头一项，**子菜单改为恒建**——空序列也进得去，编辑项正是从经典配方到任意序列的门）、`= ` 预设（`90,1500,5400` 整条替换档位列表，入口在「时长预设」头一项）。校验与落盘复用配置文件同一把尺子（`parse_span_list` 提为 `pub(crate)`），非法缀 `?` 整条不采信；提交后 `SyncConfig` 当场重建两个子菜单——顺带收掉一个真差距：`SyncConfig` 处理器此前只回填 `pomo_seq` 不回填 `presets`（预设列表是启动快照），这批补上了。热加载提示文案同步修正：预设/分段已热，要重启的只剩 GIF 动图与窗口位置。空回车四个模式一律不动作（行留着、配置不碰）。这一批合计 **+3.2 KB**（693 360 → 696 552，仅 Wayland 684 360 → 687 584），测试 243 → **247** 项。§5.2 那条"取色器对话框 L"的 HEX 输入半边就此闭合；屏幕取色（要 portal）维持不做。
