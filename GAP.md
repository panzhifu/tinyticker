...
>
> **取证基准**
> - tinyticker：工作区版本（含未提交的 `src/effect.rs` / `src/text.rs` / `src/ipc.rs`），163 项内联测试全通过；`cargo build --release` 实测 609 240 B（0.581 MiB），`--no-default-features` 600 016 B（0.572 MiB）。本文档写完后又长了字形层、套接字与这一批格式/图标项，见 §5.1、§九 与各表的"已做"标注。
...
| | tinyticker | Catime |
|---|---|---|
| 语言 / 体量 | 11.7 k 行 Rust，0.58 MB | 99 k 行 C，995 KB（32 位） |
...
8. **体积**：0.57 / 0.58 MB 两个产物（600 016 / 609 240 字节），含**两整套**手写窗口系统客户端 + 托盘 + 时区 + GIF + 特效管线 + 字形层 + 单实例套接字 + 配置热加载，且二进制里不含任何字体数据；995 KB 那份是 32 位且依赖 OS 的 WIC/audio/GDI+ 全家桶。X11 后端整份只值 9 224 B（609 240 − 600 016，本次实测），这就是"零依赖"在体积上的实际价格。
...
> 2026-09-24 更新：#27（CJK / TTF，原估 XL）、#16（单实例 + 参数转发，原估 M）、**#15（百分秒显示，原估 S）**、**#2（网络速率图标）**、**#4 + #14（补零三档 / 显示秒 / 「时间格式」子菜单）**、**#7（配置原子写）**、**#18（隐藏 / 显示挂件）**、**#17（配置热加载，实测 XS：走 stat 轮询而不是 inotify）**、**#3 + #5 + #6 + #8（30 条数字色 / `timeout_text` / `t` 后缀与 `d` 单位 / 缩放上限）**已做完。第一梯队 #1-#14 里现在只剩 #1、#9、#10、#11、#12、#13 六项没动，全是 XS。
>
...
>
> **下一步建议**：#19 编辑态一档、#20 番茄钟序列 3 段 → ≤10 段、#21 动画速率随负载、#23 数字图标档，以及第一梯队剩下的 #1/#9/#10/#11/#12/#13 六项 XS。三条根因里 **R2 的状态行那半边已闭合**，R1（键盘）与 R3（进程外能力）未动。
>
> 这一批（#15 + #2 + #4 + #14 + #7 + #18 + #17 + #3 + #5 + #6 + #8）合计 **+16 KB**：含 X11 的默认构建 592,816 → 609,240 字节，仅 Wayland 600,016 字节。另外顺手修了一个后端选择的老毛病：`WAYLAND_DISPLAY=`（空值，某些启动器会留）以前算"在 Wayland 上"，于是既不连 Wayland 也不退回 X11，直接报错退出——现在空值按"没有"处理（`main.rs:run_backend`）。
