...
>
> **取证基准**
> - tinyticker：工作区版本（含未提交的 `src/effect.rs` / `src/text.rs` / `src/ipc.rs`），135 项内联测试全通过；`cargo build --release` 实测 592 080 B（0.565 MiB），`--no-default-features` 583 512 B（0.556 MiB）。本文档写完后又长了字形层与套接字两层，见 §5.1 与 §九 的"已做"标注。
...
| | tinyticker | Catime |
|---|---|---|
| 语言 / 体量 | 10.4 k 行 Rust，0.57 MB | 99 k 行 C，995 KB（32 位） |
...
|---|---|---|---|
| **百分之一秒显示** | `TimeComponents.centiseconds`（`include/drawing/drawing_time_format.h:15-20`），`(ms%1000)/10`；`CLOCK_SHOW_MILLISECONDS`（`src/config/config_defaults.c:59`）+ 专用热键 | 只到整秒。`format_time` 输出 `45s` / `m:ss` / `h:mm:ss`（`render.rs:35-47`） | 无新机制：`tick_at` 已经按真实时刻算差值（`timer.rs:292-310`），只差把余下的亚秒部分留在状态里 + 心跳从 200 ms 降到 20 ms。**成本 S**，代价是 CPU 占空比升高约 10 倍（当前动画特效已经在跑 50 ms） |
| **自适应心跳阶梯** | `GetTimerInterval()`：毫秒 20 / 动画色 66 / 时钟带秒 250 / 计时 100 / 空闲 1000（`src/config/config_misc_display.c:76-84`），且 ≤33 ms 时改用 `timeSetEvent` + `timeBeginPeriod` | 两档常量：`DRAW_INTERVAL` 200 ms、`ANIM_INTERVAL` 50 ms，由 `Widget::tick_interval()` 选（`widget.rs:27-34`） | 已有 `tick_interval()` 这个挂载点，加档即可。**成本 XS** |
| **取整的非对称性** | 隐藏百分秒时倒计时用 **ceil**（`(remainingMs+999)/1000`），显示百分秒时用 floor（`drawing_time_format.c:100-105`）——为了不出现"开局先跳 1 秒" | 整数秒减法，天然不跳 | 只在做了百分秒时才成为问题。合并进上一项 |
| **暂停不计时** | 恢复时把暂停时长同时加到 `g_target_end_time` 和 `g_start_time`（`timer.c:247-266`） | 暂停即冻结 `secs`，等价 | **已对齐** |
| **休眠/唤醒补时** | QPC 与 `GetTickCount64` 两条时间线比对，差 ≥200 ms 就平移截止时间（`timer.c:75-107`） | `tick_at` 按墙钟差值一次补齐整秒（`timer.rs:292-310`），但只补秒 | 我们那套按秒补齐的逻辑做毫秒显示后要改成按毫秒。**成本 S**（与百分秒同批） |
...
|---|---|---|---|
| 补零策略 | 三档 `TimeFormatType{DEFAULT, ZERO_PADDED, FULL_PADDED}`（`config_types.h:32-39`），按时/分/秒三档量级分别展开，注释写明是为了"防止视觉抖动"（`timer/timer.h:7`） | 单一固定式（`render.rs:35-47`） | **XS**：`format_time` 加一个 enum 参数 |
| 隐藏秒 | `CLOCK_SHOW_SECONDS`（默认 **FALSE**，`config_defaults.c:57`）；时钟模式下关秒时连百分秒一起压掉（`drawing_time_format.c:255-263`） | 恒显示秒 | **XS** |
...
| 隐藏/显示 | `HOTKEY_TOGGLE_VISIBILITY` + 菜单项，`NO_DISPLAY` 启动模式（`startup_mode.c:31`） | ❌ 只能退出 | **S**：托盘加"隐藏"，`Hide` 是 SNI 标准状态；后端加一条 control 通道即可（和 §七的热加载共用一条管道） |
...
| 配置自愈 | 版本变更时**重建 + 回填**（逐 recognised key 拷回，未知键丢弃），含命名迁移规则表；校验失败的值会**写回**磁盘；所有写入走临时文件 + `MoveFileEx` 原子替换 + 命名互斥量，且几乎每个 `WriteConfig*` 都先比较"运行值 == 磁盘值"再决定写不写（为了避免自己触发自己的 watcher） | 未知键静默忽略、非法值回落默认（`config.rs:238-245,360`）；退出时整体重写 | 我们已经在"watcher 不存在"这一点上自洽。**只有原子写值得补**：掉电时配置可能被截断。**XS**（写 tmp + `rename`），建议做 |
...
8. **体积**：0.56 MB / 0.57 MB 两个产物，含**两整套**手写窗口系统客户端 + 托盘 + 时区 + GIF + 特效管线 + 字形层 + 单实例套接字，且二进制里不含任何字体数据；995 KB 那份是 32 位且依赖 OS 的 WIC/audio/GDI+ 全家桶。X11 后端整份只值 8 568 B（592 080 − 583 512，本次实测），这就是"零依赖"在体积上的实际价格。
...
| 14 | 外观 ▸ 里再挂「格式 ▸」（时补三档 / 显示秒 / 百分秒开关） | XS | 与 #5、#16 同一批落点，先给开关后补实现也行 |

### 第二梯队：一天到几天，收益立竿见影

| # | 项 | 档 | 说明 |
|---|---|---|---|
| 15 | **百分之一秒显示** | S | 内核已是墙钟差值制（`timer.rs:292-310`），只差保留亚秒余量 + 20 ms 心跳档 + 格式化。秒表用途的直接升级，CPU 占空比约 ×10 |
...
| 18 | 隐藏 / 显示挂件 | S | SNI `Status` 属性 + #16 的通道 |
...
> 2026-09-24 更新：#27（CJK / TTF，原估 XL）与 #16（单实例 + 参数转发，原估 M）已做完。#16 就是 #17 / #18 需要的那条反向通道，所以**下一步直接是 #15 百分秒显示**，然后 #17 配置热加载（inotify 读端 + 复用这条通道）、#18 隐藏/显示。三条根因里 **R2 的状态行那半边已闭合**，R1（键盘）与 R3（进程外能力）未动。
