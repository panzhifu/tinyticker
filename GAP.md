...
>
> **取证基准**
> - tinyticker：工作区版本（含未提交的 `src/effect.rs` / `src/text.rs` / `src/ipc.rs`），171 项内联测试全通过；`cargo build --release` 实测 624 096 B（0.595 MiB），`--no-default-features` 615 016 B（0.587 MiB）。本文档写完后又长了字形层、套接字与这一批格式/图标项，见 §5.1、§九 与各表的"已做"标注。
...
8. **体积**：0.59 / 0.60 MB 两个产物（615 016 / 624 096 字节），含**两整套**手写窗口系统客户端 + 托盘 + 时区 + GIF + 特效管线 + 字形层 + 单实例套接字 + 配置热加载，且二进制里不含任何字体数据；995 KB 那份是 32 位且依赖 OS 的 WIC/audio/GDI+ 全家桶。X11 后端整份只值 9 080 B（624 096 − 615 016，本次实测），这就是"零依赖"在体积上的实际价格。
...
>
> **下一步建议**（第一梯队已清空）：#19 编辑态一档、#20 番茄钟序列 3 段 → ≤10 段、#21 动画速率随负载、#22 CSS 颜色名与 `rgb()`、#23 数字图标档。三条根因里 **R2 的状态行那半边已闭合**，R1（键盘）与 R3（进程外能力）未动。
>
